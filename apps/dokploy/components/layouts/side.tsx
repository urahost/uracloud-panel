...
function SidebarLogo() {
  ...
  // Variables pour la card Version Gratuite
  const hasActiveStripeSub = !!user?.user?.hasActiveSubscription;
  ...
  return (
    <>
      ...
      {user && !user.user?.enablePaidFeatures && !hasActiveStripeSub && (
        <div
          className={cn(
            "mb-4 rounded-xl bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20 transition-all duration-300 mt-4 group hover:from-primary/10 hover:to-primary/15",
            state === "collapsed" ? " flex items-center justify-center" : "p-4"
          )}
        >
          {state === "collapsed" ? (
            // Version iconique pour sidebar fermée
            <Link href="/dashboard/settings/billing">
              <div className="p-2 rounded-lg bg-gradient-to-br from-primary to-primary/80 hover:from-primary/90 hover:to-primary transition-all duration-300 cursor-pointer group">
                <Sparkles className="size-5 text-primary-foreground animate-sparkle group-hover:scale-110 transition-transform duration-300" />
              </div>
            </Link>
          ) : (
            // Version complète pour sidebar ouverte
            <>
              <div className="flex items-center gap-2 text-primary mb-3">
                <Sparkles className="size-4" />
                <span className="font-semibold text-sm">Version Gratuite</span>
              </div>
              <div className="text-xs text-muted-foreground mb-3">
                {user.user?.projectsCount ?? 0}/{user.user?.projectLimit ?? 1}{" "}
                projets • {user.user?.servicesCount ?? 0}/
                {user.user?.serviceLimit ?? 2} services
              </div>
              <div className="text-xs text-muted-foreground leading-relaxed mb-3">
                Débloquez le potentiel complet avec un plan premium
              </div>
              <Link href="/dashboard/settings/billing" className="w-full block">
                <Button
                  size="sm"
                  className="w-full text-xs bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary group-hover:shadow-md transition-all duration-300"
                >
